# Using Decision Trees for Random Forest
#include <iostream>
#include <vector>
#include <cstdlib>
#include <ctime>
...
public:
    RandomForest(int n_trees) : num_trees(n_trees) {
    ...
    void fit(int* X, int* y, int num_samples, int num_features, int max_depth) {
        ...
        for (int i = 0; i < num_trees; ++i) {
            int bootstrap_X[num_samples * num_features];
            int bootstrap_y[num_samples];
            for (int j = 0; j < num_samples; ++j) {
                int idx = rand() % num_samples;
                for (int k = 0; k < num_features; ++k) {
                    bootstrap_X[j * num_features + k] = X[idx * num_features + k];
                }
                bootstrap_y[j] = y[idx];
            }
            trees[i].fit(bootstrap_X, bootstrap_y, num_samples, num_features, max_depth);
        }
    }
    ...
    double predict(int* X, int num_features) {
        int votes[2] = {0, 0};
        for (int i = 0; i < num_trees; ++i) {
            int prediction = trees[i].predict(X, num_features);
            votes[prediction]++;
        }
...
int main() {
    ...
    forest.fit(X, y, num_samples, num_features, max_depth);

    int test_point[] = {40, 1, 0};
    std::cout << "Prediction: " << forest.predict(test_point, num_features) << std::endl;
